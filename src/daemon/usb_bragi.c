...
void bragi_fill_input_eps(usbdevice *kb) {
  ...
  // Ideally, we'd read them from the OS and make sure they are valid instead of
  // guessing
  int offset = 1;
  if (IS_HARPOON_WL(kb))
    offset = 2;
...
int bragi_usb_write(usbdevice *kb, void *out, int len, int is_recv,
  ...
  // If this device has a parent, route all IO through it instead, setting our
  // id
  if (kb->parent) {
    unsigned char *pkt = out;
    pkt[0] |= kb->bragi_child_id;
    res = os_usb_interrupt_out(kb->parent, kb->parent->bragi_out_ep,
                               kb->parent->out_ep_packet_size, out, file, line);
  } else {
    res = os_usb_interrupt_out(kb, kb->bragi_out_ep, kb->out_ep_packet_size,
                               out, file, line);
  }
  // Unlock on failure
  if (is_recv && res < 1)
...
int bragi_usb_read(usbdevice *kb, void *in, int len, int dummy,
                   const char *file, int line) {
  BRAGI_PKT_SIZE_CHECK(len, file, line);
  ...
  if (condret != 0) {
    ...
    if (condret == ETIMEDOUT)
      ckb_warn_fn("ckb%d: Timeout while waiting for response", file, line,
