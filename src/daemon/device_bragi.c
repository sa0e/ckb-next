...
void *bragi_poll_thread(void *ctx) {
  ...
  int ret;
  while (
      !(ret = clock_nanosleep(CLOCK_MONOTONIC, 0, &bragi_poll_delay, NULL))) {
    ...
    if (kb->active) {
      ...
      if (usbrecv(kb, poll_pkt, sizeof(poll_response), poll_response)) {
        if (poll_response[1] != 0x12)
          ckb_err("ckb%d: Invalid bragi poll response (0x%hhx)",
                  INDEX_OF(kb, keyboard), poll_response[1]);
      } else {
        ckb_err("ckb%d: Error sending bragi poll packet",
                INDEX_OF(kb, keyboard));
      }
    }
    queued_mutex_unlock(dmutex(kb));
  }
  ckb_info("ckb%d: Bragi poll thread shutting down due to %d (%s)",
           INDEX_OF(kb, keyboard), ret, strerror(ret));
  return NULL;
}
...
static int setactive_bragi(usbdevice *kb, int active) {
  ...
  if (bragi_set_property(kb, BRAGI_MODE, active)) {
    ckb_err("ckb%d: Failed to set device to %s mode", ckb_id,
            (active == BRAGI_MODE_SOFTWARE ? "SW" : "HW"));
    return 1;
  }
...
#warning                                                                       \
    "FIXME: On idle we should only propagate it if we're the last device left active"
  if (kb->parent) {
  ...
  // The daemon will always send RGB data through handle 0 (), so go ahead and
  // open it
  uchar res = BRAGI_RES_LIGHTING;
  // K100 Air uses the alt lighting resource
  if (IS_MONOCHROME_DEV(kb))
    res = BRAGI_RES_LIGHTING_MONOCHROME;
  else if (IS_K100_AIR(kb))
    res = BRAGI_RES_ALT_LIGHTING;
  int light = bragi_open_handle(kb, BRAGI_LIGHTING_HANDLE, res);
  if (light < 0)
    return light;

  // If we forced alt lighting, we must also set the correct updatergb function
  if (res == BRAGI_RES_ALT_LIGHTING)
  ...
  if (light == 0x01 || light == 0x06) {
    // Some K100s have been observed to return either 0x01 or 0x06 which
    // probably means "not supported" If we get either response, we instead try
    // to open the alt rgb lighting resource
    ckb_warn("ckb%d: Bragi light init returned not supported", ckb_id);
    light =
        bragi_open_handle(kb, BRAGI_LIGHTING_HANDLE, BRAGI_RES_ALT_LIGHTING);
    if (light < 0)
      return light;

    if (light) {
      ckb_err("ckb%d: Bragi alt light init returned error 0x%hhx", ckb_id,
              (uchar)light);
    } else {
      ...
      // We don't yet know if this is K100 specific or if it has to be opened
      // along with the alt one
      light = bragi_open_handle(kb, BRAGI_2ND_LIGHTING_HANDLE,
                                BRAGI_RES_LIGHTING_EXTRA);
      if (light < 0)
        return light;

      if (light) {
        ckb_err("ckb%d: Bragi extra light init returned error 0x%hhx", ckb_id,
                (uchar)light);
      } else {
        // FIXME: Figure out what this is. It is definitely lighting related.
        // It has been observed to break lighting on specific keys (such has the
        // '3' key) but only on a specific K100 (fw 0.32.6, bld 0.10.45) It
        // remains commented out for now
        /*uchar pkt2[BRAGI_JUMBO_SIZE] = {
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // header
            0x1b, 0x00, 0x20, 0xe7, 0xca, 0x2f, 0x88, 0x9f, 0xff, 0xff, 0x00,
        0x00, 0x00, 0x00, 0x09, 0x00, 0x00, 0x01, 0x00, 0x0d, 0x00, 0x01, 0x01,
        0x00, 0x04, 0x00, 0x00, 0x08, 0x06, 0x02, 0x48, 0x00, 0x00, 0x00, 0x2b,
        0x00, 0x00, 0x00, 0x00, 0x00, 0xff, 0x01, 0x00, 0x00, 0x00, 0xff, 0x02,
        0x00, 0x00, 0x00, 0xff, 0x72, 0x00, 0x00, 0x00, 0xff, 0x80, 0x00, 0x00,
        0x00, 0xff, 0x89, 0x00, 0x00, 0x00, 0xff, 0xb6, 0x00, 0x00, 0x00, 0xff,
        0xb7, 0x00, 0x00, 0x00, 0xff, 0xb8, 0x00, 0x00, 0x00, 0xff, 0xb9, 0x00,
        0x00, 0x00, 0xff, 0xba, 0x00, 0x00, 0x00, 0xff, 0xbb, 0x00, 0x00, 0x00,
        0xff, 0xbc, 0x00, 0x00, 0x00, 0xff, 0xbd, 0x00, 0x00, 0x00, 0xff, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00,
        };

        if(bragi_write_to_handle(kb, pkt2, BRAGI_2ND_LIGHTING_HANDLE,
        sizeof(pkt2), 0x48)) return 1;*/
      }
    }
  } else if (light) {
    ckb_err("ckb%d: Bragi light init returned error 0x%hhx", ckb_id,
            (uchar)light);
  }
  return 0;
}
...
static inline uint32_t bragi_fwver_bswap(uint32_t fwv) {
  return ((((fwv) & 0xff000000) >> 24) | (((fwv) & 0x00ff0000) >> 8) |
          (((fwv) & 0x0000ff00) << 8) | (((fwv) & 0x000000ff) << 24)) >>
         8;
}
...
static int start_bragi_common(usbdevice *kb) {
  ...
  // Check if we're in HW mode, and if so, switch to software in order to read
  // the properties/handles
  int64_t prop = bragi_get_property(kb, BRAGI_MODE);
  if (prop < 0) {
    ckb_fatal("ckb%d: Couldn't get bragi device mode. Aborting",
              INDEX_OF(kb, keyboard));
    return 1;
  }
  ...
  // Read FW versions
  kb->fwversion = kb->bldversion = kb->radioappversion = kb->radiobldversion =
      UINT32_MAX;
  ...
  kb->features |= FEAT_ADJRATE;
  // Enable HW load for K100 devices, disable for others (as Bragi HW load isn't
  // fully implemented/tested on all)
  if (IS_K100(kb))
  ...
  if (IS_WIRELESS_DEV(kb)) {
    ...
    for (int i = 0; i < PAIR_ID_SIZE; i++)
      snprintf(str + i * 3, sizeof(str) - i * 3, "%02hhx ",
               kb->wl_pairing_id[i]);
...
int start_keyboard_bragi(usbdevice *kb, int makeactive) {
  ...
  if (kb->layout != LAYOUT_ANSI && kb->layout != LAYOUT_ISO &&
      kb->layout != LAYOUT_ABNT) {
    ckb_warn(
        "Got unknown physical layout byte value %" PRId64
        ", please file a bug report mentioning your keyboard's physical layout",
        prop);
...
int cmd_active_bragi(usbdevice *kb, usbmode *dummy1, int dummy2, int dummy3,
                     const char *dummy4) {
  (void)dummy1;
  (void)dummy2;
  (void)dummy3;
...
int cmd_idle_bragi(usbdevice *kb, usbmode *dummy1, int dummy2, int dummy3,
                   const char *dummy4) {
  (void)dummy1;
  (void)dummy2;
  (void)dummy3;
...
void bragi_get_battery_info(usbdevice *kb) {
  ...
  if (stat < 0 || chg < 0) {
    ckb_err("ckb%d: Failed to get bragi battery properties",
            INDEX_OF(kb, keyboard));
    return;
  }
...
void bragi_delay(usbdevice *kb, delay_type_t type) {
  ...
  // Since we use usbrecv for everything, the devices tell us when they are
  // ready to handle another packet.
  return;
}

static void bragi_set_device_keys(usbdevice *kb, unsigned char *pairing_id,
  ...
  bragi_write_to_handle(kb, pkt, BRAGI_GENERIC_HANDLE, sizeof(pkt),
                        sizeof(uchar) * 8);
  bragi_close_handle(kb, BRAGI_GENERIC_HANDLE);
...
void cmd_pair_bragi(usbdevice *kb, usbmode *mode, int dummy1, int dummy2,
                    const char *to) {
  (void)mode;
  (void)dummy1;
  (void)dummy2;
