...
static inline int rgbcmp(const lighting *lhs, const lighting *rhs,
                         const size_t zones, const size_t led_offset) {
  return memcmp(lhs->r + led_offset, rhs->r + led_offset, zones) ||
         memcmp(lhs->g + led_offset, rhs->g + led_offset, zones) ||
         memcmp(lhs->b + led_offset, rhs->b + led_offset, zones);
}

#define LED_CASE_M(product, count)                                             \
  __LED_CASE(product, count, N_MOUSE_ZONES_EXTENDED)
#define LED_CASE_K(product, count) __LED_CASE(product, count, (LED_MOUSE - 1))

#define __LED_CASE(product, count, limit)                                      \
  case product:;                                                               \
    static_assert(count <= limit, "count must be equal or less than " #limit   \
                                  " for dev " #product);                       \
    return count
...
static inline size_t bragi_led_count(usbdevice *kb) {
  ...
  switch (kb->product) {
    ...
    LED_CASE_K(P_K100_AIR_WIRED, 123);
...
static int updatergb_bragi(usbdevice *kb, int force, const size_t led_offset) {
  ...
  // Since the blank pkt is used to check if the lights are off, we need to make
  // sure it's sufficiently large
  static_assert(LED_MOUSE <= sizeof(pkt),
                "pkt is not large enough to check if all zones are off");
  // Switch LEDs off if its all black, because being able to just switch them
  // off even in hw mode is really nice
  ...
  static_assert(sizeof(pkt) >= 7 + N_KEYS_EXTENDED * 3,
                "Bragi RGB packet must be large enough to fit all possible "
                "zones in the keymap");
  ...
  if (!IS_MONOCHROME_DEV(kb)) {
    ...
    memcpy(pkt + 7 + CPY_SZ(r) + CPY_SZ(g), newlight->b + led_offset,
           CPY_SZ(b));
  }
  ...
  // This is done to prevent a delay when turning the lights off, caused by slow
  // HW. There seems to be no way to prevent the delay when turning the lights
  // back on.
...
static inline int updatergb_alt_bragi(usbdevice *kb, int force) {
  ...
  if (bragi_write_to_handle(kb, pkt1, BRAGI_LIGHTING_HANDLE, sizeof(pkt1),
                            3 * zones + BRAGI_ALT_RGB_HEADER))
    return 1;
