...
// Key information
struct Key {
  // Name stored in settings (this is here due to a bad design decision - it
  // will be removed later)
  const char *_storageName;
  inline const char *storageName() const {
    return _storageName ? _storageName : name;
  }
  // Key name
...
public:
  ...
  inline static bool isISO(Layout layout) {
    return layout != US && layout != US_DVORAK && layout != PL;
  }
  ...
  // Type of device
  inline static bool isKeyboard(Model model) {
    return !isMouse(model) && !isMousepad(model) && !isHeadsetStand(model) &&
           model != NO_MODEL;
  }
  inline bool isKeyboard() const { return isKeyboard(keyModel); }
  inline static bool isMouse(Model model) {
    return model == M55 || model == M65 || model == SABRE ||
           model == SCIMITAR || model == HARPOON || model == GLAIVE ||
           model == KATAR || model == KATARPROXT || model == M65E ||
           model == M95 || model == IRONCLAW || model == NIGHTSWORD ||
           model == DARKCORE || model == DARKCORERGBPRO ||
           model == IRONCLAW_W || model == GLAIVEPRO;
  }
  inline bool isMouse() const { return isMouse(keyModel); }
  inline static bool isMousepad(Model model) {
    return model == POLARIS || model == MM700;
  }
  ...
  inline static bool hasLights(Model model) {
    return !(model == M95 || model == K66 || model == BRAGI_DONGLE);
  }
  ...
  inline QString name() const {
    return (strModel() + " " + strLayout()).toUpper();
  }
  ...
  inline Key key(const QString &name) const {
    return _keys.value(name, emptyKey);
  }
  ...
  inline QHash<QString, Key>::const_iterator begin() const {
    return _keys.cbegin();
  }
  inline QHash<QString, Key>::const_iterator end() const {
    return _keys.cend();
  }
